...
Créer des couches de dessin | Autorise l'utilisateur à créer des couches de dessin temporaires
Importer des donner | Autorise l'utilisateur à importer des données SIG
Exporter des données | Autorise l'utilisateur à exporter des données. Si cette autorisation n'est pas attribuée à l'utilisateur, ce dernier sera tout de même en mesure d'exporter ses données importées ou les couches de dessins qu'il aura créés.
Utiliser la calculette des enjeux | Autorise l'utilisateur à exploiter la calulette des enjeux. La calculette des enjeux retournera toutes les couches sur lesquelles elle se base (couche de référence et d'observation) même si l'utilisateur n'est pas autorisé à consulter ce type de données.
...
Les autorisations sont cumulatives tout comme les contraintes spécifiques qui pourraient être déclarées au niveau des permission « GET_REF_LAYER », « EDIT_REF_LAYER » et « GET_OBS_DATA ».
Autrement dit, l’utilisateur possède les droits des deux groupes.

Par exemple, un utilisateur associé à un premier groupe n’ayant que le droit d’afficher la couche de référence "A" et associé à un second groupe ne pouvant afficher que la couche de référence "B" pourra afficher la couche "A" et / ou "B". 

Dans le cas où le deuxième groupe ne serait pas soumis à une liste fermée de couche (champ « group_authorization_constraint » vide) alors l’utilisateur peut consulter toutes les couches malgré la contrainte appliquée au premier groupe.

C’est la même logique pour la permission d’édition des couches de référence et pour l’échelle de restitution des données d’observation.
